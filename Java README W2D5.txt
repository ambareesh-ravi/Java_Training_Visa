...
------
	Mapping:
		Bi-Directional [ one-to-many and many-to-one]
		class Department {
	...
	@Id
	@Column
	@GeneratedValue ---> AUTO INCREMENT --> only for short, int and long
	@JoinColumn --> FK
	@JoinTable if manyto many

	---
	@OneToMany and @ManyToMany has the following properites/attribues
	a) cascade=CascadeType.NONE, CascadeType.ALL, CascadeType.PERSIST, CascadeType.DELETE, CascadeType.MERGE
	b) EAGER and LAZY
		by default ManyToOne is EAGER and one to Many is LAZY
	==========================
